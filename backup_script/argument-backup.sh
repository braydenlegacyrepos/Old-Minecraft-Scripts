...
if [ "$opt" = "$1" ]; then
	...
	if [ "$BACKUP_TOOL" = "rdiff-backup" ]; then
		...
		printf "Please define it as your second argument.\n"
		if [ "$2" == "" ]; then
			printf "Please define a number as your second argument like, $0 $1 1W2M7D13h\n"
			exit 0
		else
			rdiff-backup --remove-older-than $2 $DESTINATION_DIR
			printf "Attempted to remove the older diffs from your backup folder.\n"
		fi
		exit 0
	elif [ "$BACKUP_TOOL" = "tar" ]; then
		if [ "$2" == "" ]; then
			printf "Please define the amount of hours as an argument, like, $0 $1 10 for 10 hours.\n"
		else
			find $DESTINATION_DIR -name "*tar.gz" -type f -Btime +$2 -delete
		fi
	else
		printf "Backup-program: is incorrectly set, set to either rdiff-backup or tar for this script to work properly.\n"
		exit 0
else
	printf "Usage: Backup Restore Purge/Delete\n"
fi