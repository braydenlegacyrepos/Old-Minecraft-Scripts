...
WEBSITE_VERSION=`lynx -dump http://irc.donclurd.com/${check_opt}.txt`
if [ "$check_opt" = "Essentials" ]; then
...
elif [ "$check_opt" = "WorldGuard" ]; then
	...
	printf "The variable, ${update_opt}, did not match any known name.\n"
	exit 1
fi
if [ "$SERVER_VERSION" = "$WEBSITE_VERSION" ]; then
	printf "It appears your ${check_opt} version, $SERVER_VERSION, is up to date with the response from the website, $WEBSITE_VERSION.\n"
